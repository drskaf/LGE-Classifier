...
#df['aha_lge'] = np.array(aha_list) #df[aha_list].apply(lambda x:'{}'.format(np.max(x)), axis=1)
...
X_train = np.array([x // 255.0 for x in df_train['LGE']])
print(X_train.shape)
X_valid = np.array([x // 255.0 for x in df_valid['LGE']])
print(X_valid.shape)
...
#X_train = aug.flow(X_train, batch_size=2000)
#X_train = X_train.next()

v_aug = ImageDataGenerator(samplewise_center=True,samplewise_std_normalization=True)
#X_valid = v_aug.flow(X_valid, batch_size=1000)
#X_valid = X_valid.next()
...
weigth_path = "{}_my_model.best.hdf5".format("models/aha_VGG19")
...
history = my_model.fit(X_train, y_train, validation_data= (X_valid, y_valid), batch_size=BATCH_SIZE, epochs=NUM_EPOCHS,
...
with open("models/aha_VGG19.json", "w") as json_file:
